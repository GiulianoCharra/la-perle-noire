...
export const platos = [
  {
    ...
    precio: 1500,
    imagen: "../../assets/images/comidas/comida-1.png",
    cantidad: 1,
  },
  {
    nombre: "Carne con pure",
    precio: 1850,
    imagen: "../../assets/images/comidas/comida-58.png",
    cantidad: 1,
  },
];

let cantidadPlatos = 0;
let contenedorPlatos = null;

export function generarPlatos(contenedor) {
  contenedorPlatos = document.querySelector(`.${contenedor}`);
  cargarPlatos("container w-100 jc-fs plato", contenedor, platos, platoTemplate);
  calcularTotal();
}
...
function mostrarMensajeSinPlatos() {
  contenedorPlatos.innerHTML = `<p class="descripcion w-100">
      No hay platos agregados, si quieres agregar platos a tu pedido, ve a la sección de <a
            class="menu__opcion texto-resaltado"
            href="../menu/menu.html"
            >Menú</a
          > y selecciona los platos que desees.
    </p>`;
}

// Función que al hacer click en el botón aumentar cantidad aumente la cantidad de platos
...
export function cargarPlatos(cardNombre, cardContainer, platos, cardTemplate) {
  const contenedor = document.querySelector(`.${cardContainer}`).querySelector(".pedido__platos");
